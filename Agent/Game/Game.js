...
export default class Game {
    constructor(controller, gameover_callback, game_mode, random_generator, game_style) {
    ...
    */
    ...
    RollDices(avatar) {
        ...
        for(let dice_index = 0; dice_index < 6; dice_index++){
            ...
            dices_state[0].push(this.situation.player[0].dices[dice_index].state)
            dices_state[1].push(this.situation.player[1].dices[dice_index].state)
        }

    

        this.controller.MessageEnqueue(ret.func, [...ret.params, current_dices, dices_state])
    ...
    GameOver(winner) {
        if (this.isOver == false) {
            ...
            // console.log("0's HP = ", this.situation.player[0].health )
            // console.log("1's HP = ", this.situation.player[1].health )
            // console.log(this.situation.player[winner].godFavors)
            let score = undefined
            if(winner != null){
                ...
                if(used_token < 0){
                    ...
                    // console.log
                    console.log("[[ Token is low ]]", used_token)
                    winner = null
                    score = -5
                }
                else
            ...
            // console.log("gamevoer - winner : ", winner)
            // console.log(this.situation.player[0].health, this.situation.player[1].health)


            this.controller.MessageEnqueue("GameOver", [winner, score, this.situation.game_mode])
            this.situation.score_ = score
            // this.controller.MessageEnqueue("GameOver", [winner, this.situation.double_cube.score, this.situation.game_mode])

            // this.controller.MessageEnqueue("QueueEmpty", ()=>{this.gameover_callback(winner)})
            this.controller.GameOver()
            this.gameover_callback(winner, this.situation)
            // LogicKit.ResetRound(this.situation)

            // console.log("game over")
            // console.log(this.situation.player[0])
        }
    }
