...
function InitialGame() {
    ...
    // this.player[0].health = random.integer(14, 15)
    // this.player[0].token = random.integer(0,10)

    // this.player[1].health = random.integer(14, 15)
    // this.player[1].token = random.integer(0,10)
...
function StealToken(defender, steal_cnt){
    ...
    this.player[`${attacker}`].used_token += steal_cnt
...
function AddToken(user, cnt){
    ...
    this.player[`${user}`].used_token += cnt
}
...
function SetDicesDir(user, indexes, dirs){
    const weapons = ["axe", "axe", "arrow", "helmet", "shield", "steal"]
    ...
    for(let i = 0; i < dirs.length; i++){
        ...
        let index = indexes[i]
        dices[index].dir = dir
        dices[index].weapon = weapons[random.integer(0, 5)]
        // dices[index].weapon = "steal"

        if (dices[index].weapon == "axe") {
            dices[index].token = false
        }
        else {
            dices[index].token = random.integer(0, 1) == 0 ? true : false
        }
