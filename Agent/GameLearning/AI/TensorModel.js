...
export default class TensorModel {
    ...
    async Predict_Roll(state, rolled_dices){
        let promise_ = new Promise(res => {

            let chosen_mask = new Array(6).fill(false)
    
            let weapons_ = rolled_dices.weapon
            let marks_ = rolled_dices.mark
            ...
            for (let index = 0; index < 6; index++) {
                ...
                if(weapon_ != null){
                    ...
                    if(mark_ == true){
                        chosen_mask[index] = random.integer(0,8) > 2 ? true : false
                    }
                    else{
                        chosen_mask[index] = random.bool()
                    }
    ...
    async Predict_Godfavor(state, avaialable_godfavors){
        let promise_ = new Promise(res => {
            res(true)
