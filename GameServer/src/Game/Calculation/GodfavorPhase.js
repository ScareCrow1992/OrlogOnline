        ...
        if (godFavorInfo.godfavorIndex != -1){
            ...
            newStat.power = function(Situation, game) {
                ...
                if (this.token_ >= this.cost_) {
                    ...
                    superVisor.TriggerEvent.call(Situation, `${newStat.user}-use-token`, [this.cost_])
                    // console.log(`after : ${this.token_} & ${this.cost_}}`)

                    if (this.level >= 0) {
                        godFavorsPower[`${godFavorNames[godFavorsInfo[user].godfavorNameIndex]}`].call(newStat, Situation, datas, superVisor, game)
...
const extraInput = {
    ...
    "Frigg" : function(LogicKit, game, situation) {
        ...
        let extraInputData = LogicKit.GetExtraInputData(situation)
    ...
    "Odin": function(LogicKit, game, situation){
        let extraInputData = LogicKit.GetExtraInputData(situation)
        let inputInfo = LogicKit.GetInputData(situation)
        ...
        if(inputInfo){
            ...
            LogicKit.AddToken(situation, avatar, damage * tokenCoefficient)
