...
function GetMaterial(material_){
    // let noise_texture = window.experience.resources.items["perlin_noise"]
    ...
    material.onBeforeCompile = shader => {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            float mixRatio = 0.5;   // 0.0 ~ 1.0
            float threshold = 0.4;

            float strength = cnoise(vec3(vUv * 5.0, uTime));
            strength = 1.0 - abs(strength);

            vec4 empty = vec4(0.0, 0.00, 0.00, 1.0);

            float r = mixRatio * (1.0 + threshold * 2.0) - threshold;

            float mixf = clamp((strength - r)*(1.0 / threshold), 0.0, 1.0);
            
            diffuseColor = mix( diffuseColor, empty, mixf );
