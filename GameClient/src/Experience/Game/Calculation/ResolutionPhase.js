...
// dices = [[선공 주사위], [후공 주사위]]
function GetBattleInformation(sectors, superVisor) {
    let dicesCommand=[[], []]
    let dicesFormation = [[],[]]
    // console.log(sectors)
    ...
    for (let sectorIndex = 0; sectorIndex < 6; sectorIndex++) {
        ...
        superVisor.TriggerEvent(`${order[attacker]}_${attackWeapon}_battle_simulation`, [[dicesCommand[attacker][sectorIndex], dicesCommand[defender][sectorIndex]]])
        ...
        if(sectorIndex <= 3){
            superVisor.BlockAttack(order[defender], block, attackWeapon)
            superVisor.TakeDamage(order[defender], damage, attackWeapon)
        }
        else
            superVisor.StealToken(order[defender], damage)
