...
export default class Token{
    ...
    constructor(pos, anchorSign){
        if(instance != null){
            // tokenInfo를 반환한다.
            let token_info = instance.GetToken(anchorSign)
    ...
    Initialize(){
        ...
        users.forEach((usr, user_index)=>{
            for (let token_index = 0; token_index < 50; token_index++) {
                let instance_index = user_index * 50 + token_index
    ...
    GetToken(anchor){
        let info_ = this.Active_Token(anchor)
        return info_
    }


    StealedToken(stealed_token_index){
        let attacker, victim
        if(stealed_token_index < 50){
            attacker = "top"
            victim = "bottom"
        }
        else{
        ...
        this.index_queue[`${attacker}`]++
        let new_token_info = this.token_infos[this.index_queue[`${attacker}`]]
    ...
    Active_Token(anchor) {
        let user = null, index_ = null
        if (anchor < 0) {
            user = "top"
            if (this.index_queue[`${user}`] < 100 && this.index_queue[`${user}`] > 49) {
                index_ = this.index_queue[`${user}`]
                this.index_queue[`${user}`]++
            }
        }
        else {
            user = "bottom"
            if (this.index_queue[`${user}`] < 50) {
                index_ = this.index_queue[`${user}`]
                this.index_queue[`${user}`]++
            }
        }
...
class TokenInfo{
    ...
    Disappear() {
        gsap.timeline({ defaults: { duration: 0.65 } })
            ...
            .to(this.opacity, { ease: "none", color: 0.0, onComplete: () => { this.destroy() } })
    ...
    Appear(goal) {
        ...
        anim.to(this.opacity, { delay: 0.15, duration: 0.4, color: 1, ease: "none", onStart : ()=>{
    ...
    initialMove(goal, isImmediately = true) { 
        ...
        anim.to(this.opacity, { duration: 0.3, ease: "none", color: 1, onStart : ()=>{
            this.SetPosition(goal)
        } })
        // anim.to(this.opacity, { duration: 0.3, ease: "none", effect: 1 }, "<")
        anim.to(this.opacity, { duration: 0.4, ease: "none", effect: 0 })
    ...
    slideMove(goal) { 
        ...
        anim.to(this.opacity, {duration: 0.4, ease: "none", color : 0, onComplete : ()=>{
            this.SetPosition({ x: 0, y: -1, z: 15 })
        } }, "<")
        ...
        anim.to(this.opacity, { delay: 0.15, duration: 0.4, effect: 0, ease: "none", onComplete : ()=>{this.destroy_animation()} })
    ...
    Spend() {
        ...
        let anim = gsap.timeline()
            ...
            .to(this.opacity, { duration: 0.5, ease: "none", color: 0, onComplete : ()=>{
                this.SetPosition({ x: 0, y: -1, z: 15 })
            } })
            .to(this.opacity, { delay: 0.5, duration: 0.5, ease: "none", effect: 0 })
            .then(() => { this.destroy_animation() })
